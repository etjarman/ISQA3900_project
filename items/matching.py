...
def candidate_queryset(new_item):
    ...
    qs = Item.objects.filter(status=opposite, approved=True, category=new_item.category,
                             date_lost_or_found__range=(start,end))
    if new_item.building: qs = qs.filter(building__iexact=new_item.building)
...
def item_score(a,b):
    ...
    return round(s,1)
def find_matches_for(new_item):
    out = []
    for c in candidate_queryset(new_item):
        sc = item_score(new_item, c)
        if sc >= 40:
            out.append((c, sc))
